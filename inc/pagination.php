...
class pagination{
	...
	#to change the class name of the pagination nav
	...
	function show(){
		if(!$this->calculate)
		if($this->calculate())
		echo "<nav" . (!empty($this->id) ? " id=\"{$this->id}-pagination\"" : '') . " class=\"{$this->className}\"><ul>{$this->pagination}</ul></nav>\n";
	}
	function getOutput(){
		if(!$this->calculate) {
			if($this->calculate()) {
			return "<nav" . (!empty($this->id) ? " id=\"{$this->id}-pagination\"" : '') . " class=\"{$this->className}\"><ul>{$this->pagination}</ul></nav>\n";
			}
		}
	}
		...
		*/
		...
		if($lastpage > 1){
			if($this->page){
				//anterior button
				if($this->page > 1) {
					$this->pagination .= "<li><a href=\"".$this->get_pagenum_link(1)."\" class=\"first\">&larr;</a></li>";
			...
			if($this->page){
				...
				if ($this->page < $counter - 1) {
					...
					$this->pagination .= "<li><a href=\"".$this->get_pagenum_link($lastpage)."\" class=\"last\">&rarr;</a></li>";
				} else {
